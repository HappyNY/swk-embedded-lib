...
//!   Basically this class implements multicast delegate; which provides
//!  multiple associations of different events from different providers.
...
//!  you want to delete.
//!   Since this class uses linear vector internally, and allocation is
//!   performed through appending last, multiple associations that are
//!   completely equal can be bound in one delegate.
//!   No specific alignment happens during appending or removing operation.Thus
//!  every lookup operation takes O(n) complexity.
//!   Removing delegate operation is queued in internal storage when the
//!   delegate is on execution to prevent data race.
